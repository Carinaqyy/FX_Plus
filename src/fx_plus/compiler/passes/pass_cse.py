...
class LocalCSE(PassBase):
    """
    Eliminates the common subexpressions in local basic block.
    The implementation exploits the local value numbering by assigning unique
    hash value to different tensors and callees. The complexity is O(N)
    """
    ...
    def is_commutative(self, node):
        ...
        # Register commutative operations
        if node.target in [torch.ops.aten.add, torch.ops.aten.mul,
                           torch.ops.aten.ne]:
            return True
